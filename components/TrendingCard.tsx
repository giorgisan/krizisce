      ...
      >
        <div className="flex gap-4 lg:gap-3 relative z-10">
          ...
          <div className="flex flex-col min-w-0 flex-1 justify-center relative z-10 pointer-events-none">
              ...
              {/* --- DESKTOP RELATED (Tvoj originalen Avatar Stack) --- */}
              {related.length > 0 && (
                          ...
                          {related.map((r, i) => {
                              ...
                              return (
                                  <a 
                                     ...
                                     className="relative w-6 h-6 lg:w-5 lg:h-5 rounded-full bg-white dark:bg-gray-700 border border-gray-100 dark:border-gray-600 flex items-center justify-center overflow-hidden shadow-sm transition-all duration-300 hover:scale-125 hover:z-20"
        ...
        {/* --- NOVO: MOBILE RAZŠIRITEV (Podnaslov + Viri v ločenem bloku) --- */}
        {/* Vidno samo na mobile (md:hidden), uporabljaš tvoj related seznam, a vertikalno */}
        <div className="md:hidden mt-1 border-t border-gray-100 dark:border-gray-800 pt-2 relative z-20 pointer-events-auto">
             {(news as any).contentSnippet && (
                <p className="text-[13px] text-gray-600 dark:text-gray-400 line-clamp-2 mb-3 px-1 italic">
                    {(news as any).contentSnippet}
                </p>
             )}
             ...
             {related.length > 0 && (
                <div className="space-y-2">
                    <p className="text-[10px] uppercase tracking-wide text-gray-400 font-black px-1 mb-1">Poročajo tudi:</p>
                    {related.slice(0, 3).map((r, i) => {
                        const logo = getSourceLogoPath(r.source)
                        return (
                            <a 
                               key={i} href={r.link} target="_blank" rel="noopener"
                               className="flex items-center gap-3 p-2 rounded-lg bg-gray-50 dark:bg-gray-800/40 hover:bg-gray-100 transition-colors"
                               onClick={(e) => { e.stopPropagation(); logClick('open_related', { parent: news.link, url: r.link }) }}
                            >
                                <div className="w-5 h-5 rounded shrink-0 overflow-hidden bg-white dark:bg-gray-700 flex items-center justify-center">
                                    {logo ? <Image src={logo} alt={r.source} width={16} height={16} /> : <span className="text-[8px]">{r.source[0]}</span>}
                                </div>
                                <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate">{r.title}</span>
                                <span className="text-[10px] text-gray-400 ml-auto shrink-0 uppercase">{r.source}</span>
                            </a>
                        )
                    })}
                </div>
