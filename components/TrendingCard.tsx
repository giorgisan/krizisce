...
export default function TrendingCard({ news, compact = false, rank }: Props) {
  ...
  if (compact) {
    return (
      ...
      <div 
        className={`
            group relative transition-colors flex flex-col
            /* MOBILE STYLES (Card look) */
            bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700/50 p-3 mb-3 gap-3
      ...
      >
        ...
        <div className="flex gap-4 lg:gap-3 items-start relative z-10">
            ...
            <div className="flex flex-col min-w-0 flex-1 justify-start relative z-10 pointer-events-none pt-0.5">
                ...
                {(news as any).contentSnippet && (
                    <p className="md:hidden text-[12px] text-gray-500 dark:text-gray-400 line-clamp-2 leading-tight mt-1">
                        {(news as any).contentSnippet}
                    </p>
                )}

                {/* --- DESKTOP RELATED (Originalni Inline Avatar Stack) --- */}
                {related.length > 0 && (
        ...
        {/* --- MOBILE RELATED (Viri v ločeni vrstici spodaj) --- */}
        {related.length > 0 && (
            <div className="md:hidden flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-700/50 relative z-20 pointer-events-auto">
                        ...
                        return (
                            <a 
                               ...
                               className={`
                                  ...
                                  /* 80% opacity in grayscale privzeto */
                                  opacity-80 grayscale 
                                  /* Polna barva in opacity ob hoverju ali v grupi */
                                  group-hover/list:grayscale hover:!grayscale-0 hover:!opacity-100
