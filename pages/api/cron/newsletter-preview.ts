...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const prompt = `
      ...
      8. 'closing_line': (OPTIONAL) 1 sentence highlighting a specific, positive, concrete, interesting or notable fact from the ANY story to leave the reader with a final thought. AVOID generic summaries or cliches. CRITICAL TEMPORAL RULE: This is a MORNING newsletter. Do NOT use phrases like "Za konec dneva", "Včeraj", or "Za lahko noč". You MAY start the sentence with a short, morning-appropriate transition that reflects TODAY'S day of the week (e.g., "Za lepši vstop v vikend:", "Za uspešen skok v nov teden:", "Za miren četrtkovo jutro:"). If there is no specific interesting fact, completely OMIT this field.
