  ...
  return (
    <article className={`bg-white dark:bg-gray-800/40 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800/80 overflow-hidden flex flex-col group transition-all duration-300 hover:shadow-md ${isFeatured ? 'md:flex-row md:col-span-2' : 'col-span-1'}`}>
      ...
      <div className="flex-1 p-5 md:p-6 flex flex-col gap-4">
        ...
        <div className="bg-brand/[0.03] dark:bg-brand/[0.05] border-l-2 border-brand/50 p-4 rounded-r-lg">
          <div className="text-[9px] font-bold uppercase tracking-wider text-brand/60 mb-1.5">Uredniški okvir</div>
        ...
        {showSources && (
          <div className="pt-3 border-t border-gray-100 dark:border-gray-800/50 animate-in fade-in slide-in-from-top-1">
            <div className="flex flex-col gap-1">
              {item.sources?.map((source, sIdx) => (
                <div key={sIdx} className="group/item relative flex items-center justify-between py-2 px-2 -mx-2 hover:bg-gray-500/5 rounded-lg transition-colors">
                  <a href={source.url} target="_blank" rel="noopener" className="flex items-center gap-3 min-w-0 flex-1 pr-12">
                    <div className="relative w-4 h-4 shrink-0 rounded-sm overflow-hidden grayscale group-hover/item:grayscale-0 transition-all opacity-80 group-hover/item:opacity-100">
                      <Image src={getLogoSrc(source.source)} alt="" fill className="object-contain" />
                    </div>
                    <span className="text-[12px] font-normal text-gray-500 dark:text-gray-400 truncate group-hover/item:text-brand transition-colors">
                      {source.title}
                    </span>
                  </a>
                  
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-[8px] px-1.5 py-0.5 rounded-[2px] font-bold uppercase ${getToneColor(source.tone)}`}>
                      {source.tone}
                    </span>
                    <button 
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <p className="text-[13px] text-gray-500 dark:text-gray-400 mt-2 max-w-2xl leading-relaxed">
                        Analitična primerjava uredniškega okvirjanja (framing) najpomembnejših novic zadnjih 12 ur.
                      </p>
                    </div>
                    {lastUpdated && (
                        <div className="text-[11px] font-mono font-medium text-gray-400 flex items-center gap-2 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-full bg-gray-50 dark:bg-gray-800/50">
