...
function AnalysisCard({ item, setPreviewUrl }: { item: AnalysisItem, setPreviewUrl: (url: string) => void }) {
  ...
  const hasMoreSources = (item.sources?.length || 0) > 5;
  ...
  return (
    <article className="group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl overflow-hidden shadow-sm flex flex-col md:flex-row items-stretch transition-all duration-300 hover:border-gray-300 dark:hover:border-gray-600 w-full">
      ...
      {/* LEVI BLOK: Novica in Analiza (70%) */}
      <div className="w-full md:w-[65%] lg:w-[70%] p-4 sm:p-5 flex flex-col sm:flex-row gap-5 border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-700/50">
        ...
        {/* Slika: self-start prepreči raztezanje, aspect-video fiksira razmerje */}
        {item.main_image && (
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          {/* Citiran okvir - Navadna pisava (brez bold/italic), fiksiran na dnu (mt-auto) */}
      ...
      {/* DESNI BLOK: Viri (30%) - Vedno viden */}
      <div className="w-full md:w-[35%] lg:w-[30%] bg-gray-50/30 dark:bg-gray-800/10 p-4 sm:p-5 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2 relative">
              {visibleSources?.map((source, sIdx) => {
                  ...
                  return (
                      <div key={sIdx} className="group/source flex flex-col gap-1.5 p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-sm hover:border-brand/40 transition-colors w-full animate-in fade-in">
                          <div className="flex items-center gap-2.5 min-w-0">
                              
                              {/* Logo in Oko hover učinek */}
          ...
          {/* ODPRI/ZAPRI VEČ VIROV */}
          {hasMoreSources && (
              <button 
                onClick={() => setShowAllSources(!showAllSources)}
                className="mt-2 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-brand flex items-center gap-1.5 self-start transition-colors"
              >
                {showAllSources ? '✕ Skrij vire' : `↳ Pokaži vse vire (+${(item.sources?.length || 0) - 5})`}
              </button>
          )}
      </div>
