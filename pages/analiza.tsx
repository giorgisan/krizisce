...
interface SourceItem {
  ...
  title: string;
  url: string; 
  media_dna?: MediaDNA; // Opcijsko zaradi starih zapisov v bazi
  tone?: string;        // Fallback za stare zapise
}
...
// Komponenta za izris vizualnih DNK značk
function DNABadges({ dna }: { dna: MediaDNA }) {
  if (!dna) return null;

  const getSenzColor = (val: string) => {
      if (val === 'visok') return 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400';
      if (val === 'srednji') return 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400';
      return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400';
  };

  const getGapColor = (val: string) => {
      return val === 'da' ? 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400' 
                          : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400';
  };

  const getDensColor = (val: string) => {
      if (val === 'nizka') return 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400';
      if (val === 'srednja') return 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400';
      return 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400';
  };

  return (
      <div className="flex gap-1.5 shrink-0">
          <span title="Senzacionalizem" className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider ${getSenzColor(dna.sensationalism)}`}>
              S: {dna.sensationalism}
          </span>
          <span title="Informacijska vrzel (Clickbait)" className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider ${getGapColor(dna.info_gap)}`}>
              Vrzel: {dna.info_gap}
          </span>
          <span title="Informacijska gostota" className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider hidden xl:inline-block ${getDensColor(dna.info_density)}`}>
              Gostota: {dna.info_density}
          </span>
      </div>
  );
}
...
function AnalysisCard({ item, setPreviewUrl }: { item: AnalysisItem, setPreviewUrl: (url: string) => void }) {
  ...
  return (
    <article className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl overflow-hidden shadow-sm flex flex-col md:flex-row transition-colors hover:border-gray-300 dark:hover:border-gray-600 items-stretch">
      ...
      {/* LEVI BLOK: Novica + Sinteza (60%) */}
      <div className="w-full md:w-[60%] p-4 sm:p-5 flex flex-col sm:flex-row gap-4 sm:gap-5 border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-700/50">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          {/* NOVO: Konsenzni naslov (velik font) namesto samo "topic-a" */}
          <h2 className="text-[18px] font-serif font-bold text-gray-900 dark:text-white leading-snug mb-2">
            {item.consensus_headline || item.topic}
          </h2>
          
          <p className="text-[12.5px] text-gray-500 dark:text-gray-400 leading-relaxed mb-3">
            {item.summary}
          </p>
          
          <div className="mt-auto">
              <div className="bg-brand/[0.03] dark:bg-brand/[0.05] border-l-2 border-brand/40 pl-3 py-1.5">
                <div className="text-[9px] font-bold uppercase tracking-wider text-brand mb-1">Medijski okvir (Framing)</div>
                <p className="text-[12px] text-gray-700 dark:text-gray-300 leading-relaxed font-normal">
      ...
      {/* DESNI BLOK: Viri in DNK (40%) */}
      <div className="w-full md:w-[40%] p-4 sm:p-5 bg-gray-50/40 dark:bg-gray-800/20 flex flex-col">
        <div className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-3 flex items-center justify-between">
            <span>Interpretacije medijev ({item.sources?.length || 0})</span>
            <span className="hidden xl:inline-block text-[9px] text-gray-300 dark:text-gray-600 font-normal normal-case">Medijski DNK</span>
        </div>
        ...
        <div className="flex flex-col gap-2.5 transition-all duration-300">
          {visibleSources?.map((source, sIdx) => (
              <div 
                key={sIdx} 
                className="group/source flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2.5 p-1.5 -mx-1.5 hover:bg-gray-200/50 dark:hover:bg-gray-700/50 rounded-md transition-colors"
              >
                <div className="flex items-start sm:items-center gap-2 min-w-0 flex-1">
                  <div className="relative w-[18px] h-[18px] shrink-0 mt-0.5 sm:mt-0 transition-all">
                    <Image 
                  ...
                  <a href={source.url} target="_blank" rel="noopener" title={source.title} className="text-[11.5px] font-medium text-gray-600 dark:text-gray-300 leading-tight hover:text-brand transition-colors break-words">
                    "{source.title}"
                  </a>
                </div>
                ...
                {source.media_dna ? (
                    ...
                    // Fallback za staro bazo
                    <span className="shrink-0 text-[8px] px-1.5 py-0.5 rounded border font-bold uppercase tracking-wide bg-slate-500/10 text-slate-600">
                      {source.tone}
                    </span>
                ) : null}
              </div>
        ...
        {hasMore && (
           <button 
              ...
              className="mt-4 text-[9px] font-bold uppercase tracking-widest text-brand hover:text-brand/70 transition-colors self-start flex items-center gap-1"
           >
    ...
    <>
      ...
      <main className="min-h-screen bg-[#F9FAFB] dark:bg-gray-900 pb-20">
        ...
        <div className="bg-white dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-800 py-4 md:py-8">
            <div className="max-w-[1200px] mx-auto px-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="hidden md:block text-[13px] text-gray-500 dark:text-gray-400 mt-2 max-w-3xl leading-relaxed">
                    Orodje za zmanjševanje informacijskega šuma. Sistem destilira novice v nevtralen <strong>Konsenzni naslov</strong> in analizira <strong>Medijski DNK</strong> vsakega vira. Tako lahko hitro ločite suhoparna dejstva od senzacionalizma in praznih "clickbait" obljub.
                  </p>
                </div>
