...
function ClusterGroup({ cluster, setPreviewUrl }: { cluster: { value: number, sources: SourceItem[] }, setPreviewUrl: (url: string) => void }) {
    ...
    return (
        <div 
            className={`absolute top-1/2 left-0 -translate-y-1/2 -translate-x-1/2 w-7 h-7 cursor-pointer ${isHovered ? 'z-[100]' : 'z-10'}`}
        ...
        >
            ...
            {cluster.sources.map((source, idx) => {
                ...
                const yLift = (isHovered && N > 1) ? '-12px' : '0px';
                ...
                return (
                    <div 
                        ...
                        className={`absolute top-1/2 left-1/2 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] group/pin ${isSiblingHovered ? 'opacity-20 scale-90' : 'opacity-85 hover:opacity-100 scale-100'}`}
                        style={{
                            transform: isHovered 
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="absolute inset-0 opacity-0 group-hover/pin:opacity-100 bg-white/90 flex items-center justify-center transition-opacity duration-200">
                                <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="3" className="text-brand transform scale-50 group-hover/pin:scale-110 transition-transform duration-300 ease-out">
            ...
            {!isHovered && N > 1 && (
                <div className="absolute -top-1 -right-1 md:-right-1.5 bg-brand text-white text-[7.5px] font-bold w-3 h-3 flex items-center justify-center rounded-full shadow-sm z-50">
                    {N}
                </div>
            )}
        </div>
    ...
    <article id={newsId} className={`relative mb-8 md:mb-12 group/card transition-all duration-500 ${isFocused ? 'ring-2 ring-brand shadow-xl scale-[1.005]' : ''}`}>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 rounded-xl shadow-sm flex flex-col relative overflow-visible">
        <div className="p-5 md:p-8 flex flex-col pl-8 md:pl-12">
          <h2 className="text-[20px] md:text-[24px] font-serif font-bold text-gray-900 dark:text-white leading-snug mb-4 mt-0.5">{item.consensus_headline || item.topic}</h2>
        ...
        <div className="px-6 md:px-10 py-5 md:py-7 border-t border-gray-100 dark:border-gray-700/50 bg-gray-50/50 dark:bg-[#1e293b]/20 rounded-b-xl flex flex-col">
            <SpectrumLine title="Informacija" propKey="informativnost" leftLabel="Vaba" rightLabel="Polna slika" gradient="bg-gradient-to-r from-gray-300 via-blue-300 to-blue-500 dark:from-gray-600 dark:via-blue-500/50 dark:to-blue-500" sources={item.sources} setPreviewUrl={setPreviewUrl} />
            <SpectrumLine title="Emocija" propKey="custveni_naboj" leftLabel="Nevtralno" rightLabel="Dramatizacija" gradient="bg-gradient-to-r from-emerald-300 via-amber-300 to-rose-500 dark:from-emerald-500/80 dark:via-amber-500/80 dark:to-rose-500/80" sources={item.sources} setPreviewUrl={setPreviewUrl} />
            <SpectrumLine title="Interpretacija" propKey="pristranskost" leftLabel="Samo dejstva" rightLabel="Uredniški spin" gradient="bg-gradient-to-r from-teal-300 via-orange-300 to-red-500 dark:from-teal-500/80 dark:via-orange-500/80 dark:to-red-500/80" sources={item.sources} setPreviewUrl={setPreviewUrl} />
        </div>
      </div>
    </article>
  )
