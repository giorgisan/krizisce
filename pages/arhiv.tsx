...
export default function Home({ initialNews }: Props) {
  ...
  const [selectedSource, setSelectedSource] = useState<string>(() => {
    try {
      ...
      if (rawNew && typeof rawNew === 'string') return rawNew

      const rawOld = localStorage.getItem('selectedSources')
      if (rawOld) {
        try {
          const arr = JSON.parse(rawOld)
          const val = Array.isArray(arr) && arr[0] ? String(arr[0]) : 'Vse'
          localStorage.setItem('selectedSource', val)
