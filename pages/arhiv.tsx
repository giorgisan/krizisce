...
/* =========================================================
   KRIŽIŠČE – Home (grid <-> list)
   ---------------------------------------------------------
   Ta datoteka je razdeljena po jasnih sekcijah, da lahko
   naslednjič zlepiš SAMO del kode. Išči te markerje:

   // === [SECTION]: Imports
   // === [SECTION]: Constants & helpers
   // === [SECTION]: Shared preview (dynamic)
   // === [SECTION]: List Header
...
// === [SECTION]: List Header (sticky, blur)
function ListHeader() {
  return (
    <div
      className="sticky top-[var(--hdr-h,56px)] z-20 border-b border-gray-200/80 dark:border-gray-700/70
                 bg-white/65 dark:bg-gray-900/55 backdrop-blur-md supports-[backdrop-filter]:bg-white/45
                 dark:supports-[backdrop-filter]:bg-gray-900/40"
    >
      <div className="grid grid-cols-[72px_1fr_auto] md:grid-cols-[88px_1fr_auto] px-3 h-9 md:h-10 items-center
...
// === [SECTION]: List Row (dense, “oko” takoj ob naslovu)
function ListRow({ item }: { item: NewsItem }) {
  ...
  return (
    <>
      <li
        className="group grid grid-cols-[72px_1fr_auto] md:grid-cols-[88px_1fr_auto]
                   items-center gap-2 px-3 h-9 md:h-10
                   hover:bg-black/[0.03] dark:hover:bg-white/[0.05] transition-colors duration-100"
      >
        {/* Čas */}
        <span className="text-[11px] md:text-[12px] text-gray-500 dark:text-gray-400 tabular-nums">
          {formatDisplayTime(item.publishedAt, item.isoDate, true)}
        </span>

        {/* Naslov + oko */}
        <div className="min-w-0 flex items-center gap-1.5">
          {/* Oko – vedno tik ob naslovu; skrito na mobilnem */}
          <button
            type="button"
          ...
          <a
            ...
            className="flex-1 min-w-0 truncate text-[14px] md:text-[15px] leading-snug
                       text-gray-900 dark:text-gray-100 focus:outline-none group-hover:text-brand"
          >
            {item.title}
          </a>
        </div>
        ...
        {/* Vir */}
        <span className="ml-1 text-[11px] md:text-[12px] text-gray-600 dark:text-gray-300 inline-flex items-center gap-2 justify-self-end pr-2">
...
export default function Home({ initialNews }: Props) {
  ...
  // Po mountu prilagodi glede na širino
  useEffect(() => {
    const w = window.innerWidth
    if (w < 641) setDisplayCount(20)
    else if (w < 1025) setDisplayCount(28)
