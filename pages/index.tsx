...
export default function Home({ initialNews }: Props) {
  ...
  const filteredNews =
    filter === 'Vse' ? initialNews : initialNews.filter((article) => article.source === filter)
            ...
            {filteredNews.map((article, index) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1">
                  <div className="text-sm text-purple-400 font-semibold mb-1">{article.source}</div>
                  <h2 className="text-base font-semibold mb-2 leading-tight">{article.title}</h2>
                  <p className="text-sm text-gray-300 line-clamp-4 mb-2">{article.contentSnippet}</p>
                  <span className="mt-auto text-sm text-purple-300">Preberi več →</span>
                </div>
              </a>
            ))}
          </motion.div>
